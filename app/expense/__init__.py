...
def handle_expense_creation(form_data: dict) -> dict:
    ...
    try:
        ...
        return {
            ...
            'redirect_to': 'expense.expense_summary',
...
def validate_expense_access(expense_id: int, user) -> dict:
    ...
    try:
        ...
        if (user.id == expense.creator_id  # As creator of the expense
            or any(u.id == user.id for u in expense.users)  # Participant in the expense
            or any(expense.group_id == group.id for group in user.groups)):  # Belongs to group of the expense
